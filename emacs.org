...
#+END_SRC
*** Edit server
Start an edit server for each instance and set up the editor environment variable
#+begin_src emacs-lisp
  (setq server-name
...
#+END_SRC
Use =counsel-git-grep= instead of =project-find-regexp=
#+BEGIN_SRC emacs-lisp :noweb-ref bindings :tangle no
...
#+END_SRC
* Git
Install magit and set =C-c g= to =magit-status= and =C-c b= to =magit-blame=.
#+BEGIN_SRC emacs-lisp
  (use-package magit
    :ensure t
...
#+END_SRC
** OCaml
Use the [[https://neocaml.org/][neocaml]] toolchain with eglot.
...
TODO: find out how to get utop to work
#+END_SRC
** .Net
Add F# mode.
#+BEGIN_SRC emacs-lisp
...
#+end_src
*** Emmet
Use emmet mode for =HTML=, =JSX=, and =TSX= files.
#+begin_src emacs-lisp
...
** Rust
   ...
   Add rust mode.  Binding clippy to =C-c C-c=.
...
Use eglot since it is a part of emacs.

Add a hydra with the most common commands.

#+begin_src emacs-lisp
  (with-eval-after-load 'eglot
    (defhydra hydra-eglot (:hint nil :color teal)
      "
  ^Actions^        ^Format^         ^Refactor^     ^Diagnostics
  ^^^^^^^^-------------------------------------------------------
  _aa_ actions     _ff_ format      _rr_ rename    _dd_ buffer
  _ao_ imports     _fb_ buffer      ^  ^           _dD_ project
  _aq_ quickfix    ^  ^             ^  ^           _dn_ next
  _ae_ extract     ^  ^             ^  ^           _dp_ prev
  _ai_ inline
  -------------------------------------------------------
  _q_ quit
  "
      ...
      ("dn" flymake-goto-next-error :exit nil)
      ("dp" flymake-goto-prev-error :exit nil)
      ("q"  nil :color blue))

    (define-key eglot-mode-map (kbd "C-c e") #'hydra-eglot/body))
#+end_src
    ...
    (use-package treesit
      ...
      (when (file-exists-p "~/lib/emacs/combobulate")
        (use-package combobulate
          ...
          :load-path ("~/lib/emacs/combobulate")))))
#+end_src
* Other
** Shells
If =vterm= is available then amend it to use a prefix argument to read buffer name
#+begin_src emacs-lisp
  (when (fboundp 'vterm)
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq diary-date-insertion-form '((format "%4s-%02d-%02d" year (read month) (read day))))
#+END_SRC
...
#+END_SRC
** Re-builder
Use the =rx= macro sytnax.
#+BEGIN_SRC emacs-lisp
  (setq reb-re-syntax 'rx)
#+END_SRC
** Markdown
Enable markdown mode for markdown files.  Use =multimarkdown= if available.
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  (add-to-list 'default-frame-alist '(font . "Victor Mono Medium-12"))
#+END_SRC
Use ligatures.  Needs to be enabled globally so don't defer.
#+begin_src emacs-lisp
