...
** Rust
   ...
   [[https://github.com/rust-lang/rust-clippy][Clippy]] output in a new buffer.  TODO: Needs lots of refinement.

   #+BEGIN_SRC emacs-lisp
     (defun my/rust-clippy-diagnostics ()
       (interactive)
       (shell-command "cargo clippy" "*clippy*" "*clippy*")
       (let ((curr-buf (buffer-name))
             (root-dir (vc-root-dir)))
         (unwind-protect
             (progn
               (switch-to-buffer "*clippy*")
               (when root-dir
                 (setq default-directory root-dir))
               (compilation-mode))
           (switch-to-buffer curr-buf))))

     (bind-key (kbd "C-c C-c") #'my/rust-clippy-diagnostics 'rust-mode-map)
   #+END_SRC
